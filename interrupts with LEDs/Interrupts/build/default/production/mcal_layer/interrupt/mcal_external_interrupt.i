...
static STD_ReturnType Interrupt_INTx_Priority_Init(const interrupt_INTx_t *int_obj){
    ...
    if(((void*)0) == int_obj){
        ...
        switch(int_obj->source){
            ...
            case INTERRUPT_EXTERNAL_INT1 :
                if(int_obj->priority == INERRUPT_LOW_PRIORITY){(INTCON3bits.INT1IP=0);}
                else if (int_obj->priority == INERRUPT_HIGH_PRIORITY){(INTCON3bits.INT1IP=1);}
                else { }
                ret = (STD_ReturnType)0x01;
                break;
            case INTERRUPT_EXTERNAL_INT2 :
                if(int_obj->priority == INERRUPT_LOW_PRIORITY){(INTCON3bits.INT2IP=0);}
                else if (int_obj->priority == INERRUPT_HIGH_PRIORITY){(INTCON3bits.INT2IP=1);}
                else { }
                ret = (STD_ReturnType)0x01;
                break;
            default : ret = (STD_ReturnType)0x00;
